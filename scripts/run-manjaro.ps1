...
# clean up
ssh -t -p 1111 zwhitchcox@localhost "zetup id delete --all"
...
ssh -t -p 1111 zwhitchcox@localhost "chmod +x /tmp/zetup && echo ${ZETUP_VM_PASS} | sudo -S mv /tmp/zetup /bin/zetup" > $null

# run zetup on vm
ssh -t -p 1111 zwhitchcox@localhost "zetup cache set \"packages\" \"vim vscode\" && zetup id use gh/zwhitchcox:${GITHUB_PASS} && zetup use -b add-arch -p ssh github.com/zetup-sh/zetup-pkg"